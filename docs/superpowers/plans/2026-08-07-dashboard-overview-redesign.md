...
| `apps/admin/app/(dashboard)/dashboard-header.tsx` **new**                                          | `<h1>`, subtitle, and the tab-appropriate date filter. Sole writer of `?period=` / `?from=` / `?to=`, and of `?range=` on segment tabs only — `RevenueCard` keeps the Overview `?range=` toggle (see spec §1). |
