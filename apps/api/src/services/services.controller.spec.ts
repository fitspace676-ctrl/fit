...
import type { ListServicesResponse } from '@fit/types';
...
function setup() {
  const listServices = vi.fn<() => Promise<ListServicesResponse>>(() =>
    Promise.resolve({ services: [] }),
  );
  const services = { listServices } as unknown as ServicesService;
  ...
  return { controller: new ServicesController(services, portalBranch), listServices, resolve };
}
...
describe('ServicesController', () => {
  ...
  describe('GET /services', () => {
    it('rejects a missing gymId with 400 without hitting the service', async () => {
      ...
      expect(ctx.listServices).not.toHaveBeenCalled();
    });
    ...
    it('narrows a signed-in member to the branch resolved from their session', async () => {
      ...
      expect(ctx.listServices).toHaveBeenCalledWith({ gymId: 'gym-1', locationId: 'loc-home' });
    });

    it('lists every branch for an anonymous visitor', async () => {
      const result = await ctx.controller.list({ gymId: 'gym-1' });

      expect(ctx.listServices.mock.calls[0]).toEqual([{ gymId: 'gym-1', locationId: undefined }]);
    ...
    it('hands a same-gym override to the resolver and lists that branch', async () => {
      ...
      expect(ctx.listServices).toHaveBeenCalledWith({ gymId: 'gym-1', locationId: 'loc-2' });
    });
