...
interface ReportWindow {
  ...
  /** Bucket granularity for any time-series report (membership growth). */
...
function resolveWindow(range: ReportQuery['range']): ReportWindow {
  const end = new Date();
  switch (range) {
    case '7d':
...
function emptyBuckets(win: ReportWindow): Map<string, number> {
