...
// (see `apps/web/next.config.mjs`). Vitest never runs that build pipeline,
...
// element.
//
// `apps/web/vitest.config.ts` aliases `@stylexjs/stylex` to this file for
// the WHOLE @fit/web test project, not just one test's imports — any test
// that imports, directly or transitively, ANY component under `apps/web/`
...
// (grep `apps/web/` for `stylex\.[a-zA-Z]+` to recheck). Type-only imports
