...
Public-facing web app — [Next.js 15](https://nextjs.org/) (App Router) with
...
```bash
pnpm turbo run build --filter=@fit/web
pnpm turbo run dev --filter=@fit/web
```
...
- **Build Command**: `cd ../.. && pnpm turbo run build --filter=@fit/web`
