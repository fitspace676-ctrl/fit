...
export const env = validateEnv(
  z.object({
    ...
    // Platform root domain (`fit.ge`, or `localhost` in dev). The staff console is
    // served at `<slug>.<root>/admin`; the active gym is derived from the request
    // Host against this value. Unset → no tenant in scope (apex / preview URL).
