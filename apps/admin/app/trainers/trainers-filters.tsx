...
const STATUS_OPTIONS: ReadonlyArray<{ value: TrainerStatus; label: string }> = [
...
/**
 * The trainer roster filter bar: a debounced search box (name / headline) and a
 ...
 */
export function TrainersFilters({ search, status }: { search: string; status: string }) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
      ...
      <div className="sm:w-48">
        <label htmlFor="trainer-status" className="sr-only">
          Filter by status
        </label>
        <select
          id="trainer-status"
