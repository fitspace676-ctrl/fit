...
import { formatPrice, intervalLabel, intervalSuffix, sessionLabel } from '../format';
...
/**
 * The package-plan detail page (T4.11). Server-fetches `GET /admin/packages/:id`
 * and renders the identity header (name + status + popular badge), the price with
 * its billing cadence, the session count, the description, and the features list,
 * plus the write controls for `PackageWrite` staff. A `404` from the API — unknown
 * or cross-tenant id — becomes Next's `notFound()`; any other failure surfaces
 * inline.
 */
export default async function PackagePlanDetailPage({
  params,
}: {
  ...
  // Write controls (edit + deactivate) are a `PackageWrite` capability.
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="flex flex-wrap gap-x-8 gap-y-2 text-sm">
        <div className="flex flex-col gap-0.5">
          ...
          <span className="text-slate-800">{intervalLabel(plan.billingInterval)}</span>
        </div>
        <div className="flex flex-col gap-0.5">
          <span className="text-xs uppercase tracking-wide text-slate-500">Sessions</span>
          <span className="text-slate-800">{sessionLabel(plan.sessionCount)}</span>
        </div>
      </section>
