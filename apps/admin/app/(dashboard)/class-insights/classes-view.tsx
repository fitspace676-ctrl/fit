...
// leaving Classes for another tab unmounts it and the cache goes with it.
//
...
import { DemandHeatmapCard } from './demand-heatmap-card';
import { TopClassTypesCard } from './top-class-types-card';
import { PtSessionsCard } from './pt-sessions-card';
...
export function ClassesView() {
  ...
  const [data, setData] = useState<DashboardClassesResponse | null>(null);
  ...
  useEffect(() => {
    ...
    void loadClassesAction({ granularity, locationId })
      ...
      // `loadClassesAction` resolves its OWN failures into `{ ok: false }`, so this
    ...
    <div {...stylex.props(styles.page, pending && styles.pending)}>
      ...
      <div {...stylex.props(styles.workArea)}>
        ...
        {/*
          ...
          narrow column; the 24-wide heatmap does not, which is why it sits on the
          left with the trends.
        */}
