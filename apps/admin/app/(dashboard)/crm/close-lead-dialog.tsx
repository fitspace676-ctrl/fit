...
import { convertLeadAction, loseLeadAction } from './actions';

/** Which close transition the dialog performs. */
export type CloseKind = 'won' | 'lost';

/**
 * The selectable close reasons per transition (from the reference board):
 * a win explains what worked, a loss what didn't — with a free-text "other".
 * Labels come from `dialogs.reasons.<key>`.
 */
...
/**
 * The Mark-Won / Mark-Lost dialog — the only way a lead closes, so the reason
 * is always captured with the close (required on a loss, and the picker makes
 * it effectively required on a win too). "Other" opens a free-text line; the
 * stored reason is the label the staff member picked (or typed).
 */
export function CloseLeadDialog({
  ...
  function submit(): void {
    ...
    startTransition(async () => {
      const result =
        kind === 'won'
          ? await convertLeadAction(leadId, reason)
          : await loseLeadAction(leadId, reason);
      if (result.ok) {
        toast(kind === 'won' ? t('dialogs.wonDone') : t('dialogs.lostDone'), {
          tone: 'success',
          icon: 'check',
        });
        onClose();
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={kind === 'won' ? t('dialogs.wonTitle') : t('dialogs.lostTitle')}
      description={
        kind === 'won'
          ? t('dialogs.wonDescription', { name: leadName })
          : t('dialogs.lostDescription', { name: leadName })
      }
      size="md"
      footer={
        <>
