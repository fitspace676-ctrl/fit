...
/**
 * The opportunity detail's tabbed experience (T12.4): Overview (deal notes +
...
export function OpportunityTabs({
  ...
  const pendingCount = opportunity.tasks.filter((task) => !task.completed).length;
  ...
  return (
    <div {...stylex.props(styles.wrap)}>
      ...
      <div role="tabpanel">
        {active === 'overview' && <OverviewPanel opportunity={opportunity} t={t} />}
        {active === 'activity' && (
          <ActivityPanel opportunity={opportunity} canManage={canManage} t={t} locale={locale} />
        )}
        {active === 'notes' && (
          <NotesPanel opportunity={opportunity} canManage={canManage} t={t} locale={locale} />
        )}
        {active === 'tasks' && (
          <TasksPanel opportunity={opportunity} canManage={canManage} t={t} locale={locale} />
        )}
      </div>
    </div>
  );
...
/** Overview — the close reason (once closed) and the opportunity's notes. */
function OverviewPanel({ opportunity, t }: { opportunity: OpportunityDetail; t: T }) {
  return (
    <div {...stylex.props(styles.panelStack)}>
      {opportunity.status === 'WON' && opportunity.wonReason ? (
        <Card variant="default" padding={0} xstyle={styles.reasonCardWon}>
          ...
          <p {...stylex.props(styles.reasonBody)}>{opportunity.wonReason}</p>
        </Card>
      ) : null}
      {opportunity.status === 'LOST' && opportunity.lostReason ? (
        <Card variant="default" padding={0} xstyle={styles.reasonCardLost}>
          ...
          <p {...stylex.props(styles.reasonBody)}>{opportunity.lostReason}</p>
        </Card>
      ) : null}
      ...
      <Card variant="default" padding={0} xstyle={styles.card}>
        ...
        {opportunity.notes ? (
          <p {...stylex.props(styles.bodyText)}>{opportunity.notes}</p>
        ) : (
          <p {...stylex.props(styles.mutedText)}>{t('opportunityDetail.noNotes')}</p>
        )}
      </Card>
    </div>
  );
...
function ActivityPanel({
  ...
  function submit(): void {
    ...
    startTransition(async () => {
      const result = await logOpportunityActivityAction(opportunity.id, {
  ...
  return (
    <div {...stylex.props(styles.panelStack)}>
      ...
      <Card variant="default" padding={0} xstyle={styles.card}>
        ...
        <p {...stylex.props(styles.sectionHint)}>{t('opportunityDetail.activityTimelineHint')}</p>
          ...
          <ul {...stylex.props(styles.timeline)}>
            {opportunity.activities.map((activity, index) => (
              <TimelineEntry
                key={activity.id}
                activity={activity}
                isLast={index === opportunity.activities.length - 1}
                t={t}
                locale={locale}
              />
            ))}
...
function NotesPanel({
  ...
  const notes = opportunity.activities.filter((activity) => activity.type === 'NOTE');
        ...
        <Card variant="default" padding={0} xstyle={styles.card}>
          ...
          <div {...stylex.props(styles.addForm)}>
            <Textarea
              ...
              placeholder={t('opportunityDetail.addNotePlaceholder')}
...
function TasksPanel({
  ...
  const pendingTasks = opportunity.tasks.filter((task) => !task.completed);
  const completedTasks = opportunity.tasks.filter((task) => task.completed);

  function addTask(): void {
    const trimmed = title.trim();
    if (!trimmed || pending) return;
    startTransition(async () => {
      const result = await createOpportunityTaskAction(opportunity.id, {
        title: trimmed,
  ...
  function toggle(task: CrmTaskEntry): void {
    startTransition(async () => {
      const result = await toggleOpportunityTaskAction(opportunity.id, task.id, !task.completed);
      if (result.ok) {
        router.refresh();
      } else {
        toast(result.error, { tone: 'danger', icon: 'info' });
