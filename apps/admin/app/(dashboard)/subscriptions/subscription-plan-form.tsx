...
/**
 * The create / edit subscription-plan form (T8.2), laid out as the reference
 * "Billing · Plan editor": form cards on the left (Basics & pricing, Perks,
 * Visibility), a live member-facing preview on the right, and a floating
 * unsaved-changes save bar. One component serves both flows.
 *
 * Beyond the profile fields (name, description, price, currency, cadence) it
 * owns the perks editor (add-and-press-Enter rows, blank entries never leave
 * the input) and two visibility switches: "Active" (create: the initial
 * `status`; edit: staged and applied via the dedicated lifecycle action after
 * the profile PATCH) and "Mark as popular" (the emphasised plan).
 *
...
export function SubscriptionPlanForm(props: Props) {
  ...
  function onSubmit(event: React.FormEvent<HTMLFormElement>): void {
    ...
    const nextStatus: SubscriptionPlanStatus = active ? 'ACTIVE' : 'INACTIVE';

    startTransition(async () => {
      if (isEdit) {
        const result = await updateSubscriptionPlanAction(props.planId, profile);
        if (!result.ok) {
        ...
        if (nextStatus !== props.initial.status) {
          const lifecycle = await setSubscriptionPlanActiveAction(props.planId, active);
          if (!lifecycle.ok) {
            setError(lifecycle.error);
            return;
          }
        }
        router.push(`/subscriptions/${props.planId}`);
        router.refresh();
      } else {
        const result = await createSubscriptionPlanAction({ ...profile, status: nextStatus });
        if (result.ok) {
  ...
  const previewUnit = intervalSuffix(interval);
      ...
      <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-[1fr_360px]">
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label htmlFor="plan-name" className={LABEL_CLASS}>
                  Plan title<span className="text-danger-400"> *</span>
                </label>
                <input
                  id="plan-name"
                  name="name"
          ...
          <Card glow className="p-5 sm:p-6">
            ...
            {atFeatureLimit ? (
              <p className="mt-1.5 text-[11px] text-ink-400 dark:text-ink-500">
                A plan can have at most {MAX_SUBSCRIPTION_FEATURES} perks.
              </p>
            ) : null}
          </Card>
          ...
          <Card glow className="p-5 sm:p-6">
            ...
            <div className="mt-4 space-y-2.5">
              {[
                {
                  title: 'Active',
                  detail: 'Members can subscribe to this plan.',
                  on: active,
                  set: () => setActive((a) => !a),
                },
                {
                  title: 'Mark as popular',
                  detail: 'Highlights the plan with a badge.',
                  on: popular,
                  set: () => setPopular((p) => !p),
                },
              ].map((row) => (
        ...
        <div className="space-y-3 lg:sticky lg:top-[88px]">
          ...
          <p className="text-center text-[11px] text-ink-400 dark:text-ink-500">
            This is how members see the plan on the pricing page.
          </p>
        </div>
      </div>
      ...
      >
        <div className="flex items-center gap-3 rounded-card bg-ink-900 py-2.5 pl-4 pr-2.5 text-white shadow-[0_24px_60px_-16px_rgba(0,0,0,0.8)] ring-1 ring-white/10">
          ...
          <span className="text-sm font-medium text-ink-200">
            {dirty ? 'Unsaved changes' : 'New plan'}
          </span>
          ...
          <Btn type="submit" v="primary" size="sm" icon="check" disabled={pending}>
            {pending ? 'Saving…' : isEdit ? 'Save plan' : 'Create plan'}
          </Btn>
        </div>
      </div>
    </form>
